...
export const AmountInput: React.FC<AmountInputProps> = ({ value, onChange, disabled, error }) => {
  ...
  return (
    <div className="space-y-1.5">
      <label className="block text-center text-sm font-bold text-ink-black">{t.quickDeduct.page.total}</label>
      <div className="relative">
        <input
          data-testid="qd-total"
          disabled={disabled}
          inputMode="numeric"
          placeholder={t.quickDeduct.page.amountPlaceholder}
          type="text"
          value={value ? Number(value).toLocaleString(locale === "vi" ? "vi-VN" : "en-US") : ""}
